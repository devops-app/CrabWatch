  ...
  const checkAchievements = useCallback(async () => {
    try {
      ...
      if (res.newlyUnlocked.length > 0) {
        setToast(`🎉 ${t('unlockedToast', { names: res.newlyUnlocked.join(', ') })}`)
    ...
    <>
      ...
      <div className="mb-6 bg-white rounded-xl p-4 border border-ocean-100 shadow-sm">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-ocean-700">{t('overallProgress')}</span>
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-ocean-500 uppercase">{t('categoryLabel')}</label>
          <div className="flex gap-2">
            {CATEGORIES.map((cat) => (
              ...
              >
                {cat === 'ALL' ? t('all') : `${CATEGORY_ICONS[cat] || '📌'} ${t(['category', cat.toLowerCase()].join('.'))}`}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-ocean-500 uppercase">{t('status')}</label>
          <div className="flex gap-2">
            {(['all', 'unlocked', 'in_progress'] as const).map((status) => (
              ...
              >
                {status === 'all' ? t('all') : status === 'unlocked' ? t('unlocked') : t('inProgress')}
              </button>
            ))}
          </div>
        </div>
          ...
          {filtered.map((a) => (
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-between text-xs text-ocean-400">
                <span>{t('xpReward', { xp: a.xpReward })}</span>
                {a.isUnlocked && a.earnedAt && (
                  <span>
                    {t('earned', { date: fmt.formatDate(new Date(a.earnedAt)) })}
                  </span>
                )}
              </div>
              ...
              {!a.isUnlocked && (
                <div className="mt-3">
                  <div className="flex items-center justify-between text-xs text-ocean-500 mb-1">
                    ...
                    <span>
                      {t('progressCount', { current: Math.min(a.progress, a.target), target: a.target })}
                    </span>
                  </div>
