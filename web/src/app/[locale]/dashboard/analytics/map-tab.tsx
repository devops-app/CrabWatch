...
const MapTab = memo(function MapTab({
  ...
  const t = useTranslations('analytics.map')
  ...
  return (
    <div className="card">
      <div className="flex justify-between items-center mb-3">
        <div>
          <h2 className="text-xl font-semibold text-ocean-800 mb-1">{t('title')}</h2>
          ...
          <div className="absolute bottom-4 left-4 bg-white/90 rounded-lg px-3 py-2 text-xs text-gray-600">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <span className="text-gray-400 w-16">{t('filterSpecies')}</span>
                <span className="font-medium">{getSpeciesName(selectedSpecies) || t('allSpecies')}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-gray-400 w-16">{t('filterGender')}</span>
                <span className="font-medium capitalize">{genderFilter || t('allGenders')}</span>
              </div>
              {effectiveDates.from && (
                <div className="flex items-center gap-2">
                  <span className="text-gray-400 w-16">{t('filterPeriod')}</span>
                  <span className="font-medium">
                    {formatFilterDate(effectiveDates.from) || t('start')} — {formatFilterDate(effectiveDates.to) || t('now')}
                  </span>
                </div>
              )}
              <div className="flex items-center gap-2">
                <span className="text-gray-400 w-16">{t('filterCount')}</span>
                <span className="font-medium">{validMapObs.length} {t('withCoordinates')}</span>
              </div>
              {mapTotal > mapObs.length && (
                <div className="text-[11px] text-amber-700 mt-1">{t('showingFirst', { shown: mapObs.length, total: mapTotal })}</div>
              )}
            </div>
          </div>
        </div>
